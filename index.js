...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.d6eerui.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    app.post("/users", async (req, res) => {
      ...
      // If user doesn't exist in the DB, add that user
      const existingUser = await userCollection.findOne(query);
      if (existingUser) {
        return res.send({ message: "User already exists in the DB" });
      }
    ...
    app.delete("/menu/:id", verifyToken, verifyAdmin, async (req, res) => {
      ...
      const result = await menuCollection.deleteOne(query1);
      res.send(result);
    })
    ...
    //   // const menuItemIds = payments.map(order => order.menuItemId);
    //   // res.send(menuItemIds);
    ...
    app.get("/order-stats", async (req, res) => {
      const result = await paymentCollection.aggregate([
        ...
        {
          $addFields: {
            menuItemIdObj: { $toObjectId: "$menuItemId" }
          }
        },
        {
          $lookup: {
